...
const InboxReplyInstagramPlugin = ({ isConnectable }) => {
	return (
		<ChildNodes>
			<Handle
				type="target"
...
const CommentReplyInstagramPlugin = ({ isConnectable }) => {
	return (
		<ChildNodes>
			<Handle
				type="target"
...
const SendMessageInstagramPlugin = ({ id, isConnectable, data }) => {
		...
		if (value === list[2]) {
			...
			setNodes((nds) =>
				nds.map((node) => {
					if (node.id === id) {
						node.data = {
							label: "instagramplugin-sendmessage",
							ai: true,
						};
					}
...
const PostInstagramPlugin = ({ id, isConnectable, data }) => {
	const { showToast } = useToast();
	const { setNodes, getNodes } = useReactFlow();
