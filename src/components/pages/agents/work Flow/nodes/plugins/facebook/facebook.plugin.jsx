...
const FacebookPlugin = ({ id, data }) => {
	...
	return (
		<Nodes title="Facebook" type="Plugin" onDelete={deleteNode} icon={icon}>
			<div style={{ width: "100%", height: "480px" }}>
				<div className="id-input" style={{ width: "100%" }}>
					<h6 style={{ marginBottom: "3px" }}>Facebook ID</h6>
					<Inputs2
						node={true}
						type={"text"}
						label={"id"}
...
const InboxReplyFacebookPlugin = ({ isConnectable }) => {
	return (
		<ChildNodes>
			<Handle
				type="target"
...
const CommentReplyFacebookPlugin = ({ isConnectable }) => {
	return (
		<ChildNodes>
			<Handle
				type="target"
...
const SendMessageFacebookPlugin = ({ id, isConnectable, data }) => {
		...
		if (value === list[2]) {
			...
			setNodes((nds) =>
				nds.map((node) => {
					if (node.id === id) {
						node.data = {
							label: "facebookplugin-sendmessage",
							ai: true,
						};
					}
...
const PostFacebookPlugin = ({ id, isConnectable, data }) => {
	const { showToast } = useToast();
	const { setNodes, getNodes } = useReactFlow();
