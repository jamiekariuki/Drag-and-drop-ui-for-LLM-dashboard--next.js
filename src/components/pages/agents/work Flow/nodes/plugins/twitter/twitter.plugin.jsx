...
const InboxReplyTwitterPlugin = ({ isConnectable }) => {
	return (
		<ChildNodes>
			<Handle
				type="target"
...
const CommentReplyTwitterPlugin = ({ isConnectable }) => {
	return (
		<ChildNodes>
			<Handle
				type="target"
...
const SendMessageTwitterPlugin = ({ id, isConnectable, data }) => {
		...
		if (value === list[2]) {
			...
			setNodes((nds) =>
				nds.map((node) => {
					if (node.id === id) {
						node.data = {
							label: "twitterplugin-sendmessage",
							ai: true,
						};
					}
...
const PostTwitterPlugin = ({ id, isConnectable, data }) => {
	const { showToast } = useToast();
	const { setNodes, getNodes } = useReactFlow();
